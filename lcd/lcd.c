...
void LcdPrintCharS1(unsigned char x, unsigned char y,unsigned char c[])
{
    current_row = x%2;
    current_col = y%16;
    LcdScreen1[current_row][ current_col] = c;
}
...
void DisplayLcdScreen_1()
{
     unsigned char i;
    lcd_set_cursor (0,0);
    for (i = 0; i<16; i++)
        lcd_print_char_1(LcdScreen1[0][i]);
    lcd_set_cursor (1,0);
    for (i = 0; i<16; i++)
        lcd_print_char_1(LcdScreen1[1][i]);
}
